/******/ (function () {
        ...
        function FloorSelector() {
          ...
          function showPopup(target, left, top) {
            ...
            var row = document.querySelector(
              '.js-flats-table tr[data-locations~="' + target + '"]'
            );
            if (!row) return false;
            var fields = [
              "number",
            ...
            if (
              ...
              $(".status").text() == "Pēc pieprasījuma" ||
              $(".status").text() == "По запросу"
            ) {
              $("#price_tr").hide();
            } else {
              $("#price_tr").show();
          ...
          function onActionTouchstart(ev) {
            //console.log("onActionTouchstart")
          ...
          function onActionLeave(ev) {
            ...
            if (
              !ev.toElement ||
              ev.toElement.classList.contains("apt") ||
              ev.toElement.closest(".apt")
            ) {
              // console.log('no leave', ev.toElement);
              return;
            }
            if (
              ev.toElement == flatInfoPopup ||
              ev.toElement.closest(".js-selector-popup")
            ) {
              hidePopup();
            }
            var parent = this.closest(".action");
          ...
          function doAction(target) {
            ...
            // }
            hidePopup();
            var targetSlide = visualBlock.querySelector(
              ".slide." + targetPath[0]
            );
            if (!targetSlide) {
            ...
            if (targetPath[1]) {
              if (targetPath[0] == "apt" || targetPath[0] == "loft") {
                ...
                for (var i = 0; i < aptPlans.length; i++) {
                  if (aptPlans[i].classList.contains(targetPath[1])) {
                    ...
                    if (aptPlans[i].classList.contains("has-second-floor")) {
                      document
                        .querySelector(".js-floors")
                        .classList.add("has-second-floor");
                    } else {
                      document
                        .querySelector(".js-floors")
                        .classList.remove("has-second-floor");
                    }
                  } else {
                ...
                var targetFloorplan = visualBlock.querySelector(
                  ".floorplan." + targetPath[1]
                );
            ...
            try {
              for (_iterator.s(); !(_step = _iterator.n()).done; ) {
                ...
                if (mutation.type === "childList") {
                  _toConsumableArray(mutation.addedNodes).forEach(function (
                    addedNode
                  ) {
                    ...
                    if (
                      ...
                      _toConsumableArray(addedActions).forEach(function (
                        actionElement
                      ) {
          ...
          filterSliders.forEach(function (slider) {
            defaultFilterValues[slider.dataset.attr] = filterValues[
              slider.dataset.attr
            ] = {
            ...
            $(slider).slider({
              ...
              values: [
                parseInt(slider.dataset.min),
                parseInt(slider.dataset.max),
              ],
          ...
          function resetAndFilterTableByFloor(floorMin, floorMax, type) {
            if (type) {
              var apartmentCheckbox = document.querySelector(
                "#id_type_apartments"
              );
          ...
          function filterTable() {
            ...
            rows.forEach(function (row) {
              ...
              for (var attr in filterValues) {
                if (
                  !inRange(
                    parseFloat(row.dataset[attr]),
                    filterValues[attr].low,
                    filterValues[attr].high
                  )
                ) {
                  row.classList.add("filtered-out");
                  break;
            ...
            rows.forEach(function (row) {
              ...
              locations.forEach(function (location) {
                ...
                if (
                  ...
                  floorplanItem.classList.add("reserved");
                } else if (
                  floorplanItem &&
                  (status == "Sold" ||
                    status == "Pārdots" ||
                  ...
                  floorplanItem.classList.add("reserved");
                } else if (floorplanItem)
                  floorplanItem.classList.add("unavailable");
              });
            });
          }
          var typeInputs = document.querySelectorAll(
            '.js-flats input[name="flats_type"]'
          );
          ...
          function onAptFloorSelectorClick(ev) {
            ...
            var siblings = target
              .closest(".floor-selector")
              .querySelectorAll("a");
            for (var i = 0; i < plans.length; i++) {
              if (plans[i].dataset.floor != floor)
                plans[i].classList.remove("active");
              else plans[i].classList.add("active");
              if (siblings[i].dataset.floor != floor)
                siblings[i].classList.remove("active");
              else siblings[i].classList.add("active");
            }
          }
          var aptFloorSelectors = document.querySelectorAll(
            ".aptplan .floor-selector a"
          );
  ...
  \*****************************/
    ...
    /* harmony import */ var _smoothscroll_min_js__WEBPACK_IMPORTED_MODULE_1__ =
      __webpack_require__(
        /*! ./smoothscroll.min.js */ "./resources/js/smoothscroll.min.js"
      );
    /* harmony import */ var _smoothscroll_min_js__WEBPACK_IMPORTED_MODULE_1___default =
      /*#__PURE__*/ __webpack_require__.n(
        _smoothscroll_min_js__WEBPACK_IMPORTED_MODULE_1__
      );
    /* harmony import */ var _floorselector__WEBPACK_IMPORTED_MODULE_2__ =
      __webpack_require__(
        /*! ./floorselector */ "./resources/js/floorselector.js"
      );
    ...
    function App() {
      ...
      function generateAutoAnchors() {
        var slugify = __webpack_require__(
      ...
      function onReady() {
        ...
        var menuElements = document.querySelectorAll(
          ".js-mobmenu-background, .js-mobmenu-content"
        );
        var menuHandler = new _menu__WEBPACK_IMPORTED_MODULE_0__["default"](
          menuElements
        );
        ...
        if (floorSelectorElement) {
          var selector = new _floorselector__WEBPACK_IMPORTED_MODULE_2__[
            "default"
          ]();
  ...
  document.querySelectorAll('.floor-selector a').forEach(link => {
  link.addEventListener('click', function (e) {
    e.preventDefault();
    const floor = this.dataset.floor;
    document.querySelectorAll('.plan').forEach(p => p.style.display = 'none');
    document.querySelector(`.plan[data-floor="${floor}"]`).style.display = 'block';
  });
});
