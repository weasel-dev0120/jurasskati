...
class SettingsAdminController extends BaseAdminController
{
    ...
    public function save(Request $request, FileUploader $fileUploader): RedirectResponse
    {
        ...
        if ($request->hasFile('home_cover_image')) {
            $validator = Validator::make($request->all(), [
                'home_cover_image' => 'image',
            ]);
            if ($validator->passes()) {
                $file = $fileUploader->handle($request->file('home_cover_image'), 'settings');
                $this->deleteImage();
                $data['home_cover_image'] = $file['filename'];
            }
        }

        if ($request->hasFile('home_intro_image')) {
            $validator = Validator::make($request->all(), [
                'home_intro_image' => 'image',
            ]);
            if ($validator->passes()) {
                $file = $fileUploader->handle($request->file('home_intro_image'), 'settings');
                $this->deleteImage();
                $data['home_intro_image'] = $file['filename'];
            }
        }
